#!/bin/bash
# Deploy leakage-free evaluation (v4) to Lambda Cloud
# Usage: bash deploy_and_run_v4.sh <lambda-ip>
set -e

IP="${1:-$(cat /tmp/lambda_ip.txt 2>/dev/null)}"
if [ -z "$IP" ]; then echo "Usage: bash deploy_and_run_v4.sh <lambda-ip>"; exit 1; fi
...
echo "=========================================="
echo "  Leakage-Free Evaluation v4"
echo "  Target: $IP"
echo "=========================================="
...
$SCP $LOCAL/12_leakage_free_evaluation.py ubuntu@$IP:$REMOTE/
...
$SSH "pip install --quiet optuna catboost xgboost lightgbm imbalanced-learn joblib category_encoders 2>&1 | tail -2"
...
echo -e "\n[5/6] Run leakage-free evaluation (~3 hours)..."
$SSH "cd $REMOTE && python3 12_leakage_free_evaluation.py 2>&1 | tee leakage_free.log"

echo -e "\n[6/6] Download results..."
mkdir -p $LOCAL/results
$SCP "ubuntu@$IP:$REMOTE/results/leakage_free_results.json" $LOCAL/results/ 2>/dev/null || true
$SCP "ubuntu@$IP:$REMOTE/leakage_free.log" $LOCAL/ 2>/dev/null || true
