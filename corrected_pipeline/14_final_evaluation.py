"""
Phase 14: FINAL Leakage-Free Evaluation (Definitive Thesis Results)
All methods pre-June 2024. Zero data leakage.

Combines proven script-12 framework (F1=0.738) with:
  - Claim-level two-stage model (4 new features, inside CV fold)
  - Bayesian target encoding (category_encoders, inside CV fold)
  - IsolationForest contamination from train fold y.mean()
  - Cohen's d / Cliff's delta effect sizes
  - All 5 panel review bug fixes

NO focal loss, NO DART, NO class weight overrides (tested, all hurt F1).
"""
import argparse
import json
import logging
...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(), logging.FileHandler("final_evaluation.log")],
)
...
def compute_fold_isoforest(train_X, test_X, y_train=None):
    """IsolationForest fitted on train only. Contamination from train fraud rate."""
    contam = float(y_train.mean()) if y_train is not None else 0.094
...
def main():
    ...
    log.info("=" * 70)
    log.info("PHASE 14: FINAL LEAKAGE-FREE EVALUATION")
    log.info("  All cross-provider features computed inside CV folds")
    log.info("  + Claim-level model + Target encoding (additive)")
    log.info("=" * 70)
    ...
    # Load tuned params (use v3 params - proven F1=0.735 in script 12)
    # NOTE: exhaustive_params.json optimized for AUC-PR with low scale_pos_weight
    # which gives poor F1 at default threshold. v3 params are more reliable.
    tuned_params = {}
    v3_path_params = RESULTS_DIR / "best_params_v3.json"
    if v3_path_params.exists():
        with open(v3_path_params) as f:
            tuned_params = json.load(f)
        log.info(f"  Loaded v3 params ({len(tuned_params)} models)")
    else:
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(outer_cv.split(safe_X, y)):
        ...
        # Step 2d: IsolationForest (contamination from train fold)
        iso_train, iso_test = compute_fold_isoforest(X_train_df.values, X_test_df.values, y_train)
        ...
        # Step 2h: Bayesian target encoding (train only, ~20 features)
        try:
            from category_encoders import TargetEncoder
            te_cols = [c for c in diag_frac_cols[:20] if c in X_train_df.columns]
            if te_cols:
                encoder = TargetEncoder(cols=te_cols, smoothing=10, return_df=True)
                te_train = encoder.fit_transform(X_train_df[te_cols], pd.Series(y_train))
                te_test = encoder.transform(X_test_df[te_cols])
                for col in te_cols:
                    X_train_df[f"{col}_te"] = te_train[col].values
                    X_test_df[f"{col}_te"] = te_test[col].values
                log.info(f"    Added {len(te_cols)} target-encoded features")
        except ImportError:
            log.warning("    category_encoders not available, skipping target encoding")
        except Exception as e:
            log.warning(f"    Target encoding failed: {e}")

        # Step 2i: Claim-level two-stage model (4 new features)
        try:
            claim_train, claim_test = train_claim_level_model(
                full, train_providers_set, test_providers_set, pos_weight
            )
            for cdf, xdf, idx_arr in [(claim_train, X_train_df, train_idx), (claim_test, X_test_df, test_idx)]:
                prov_order = providers[idx_arr]
                cdf_reindexed = cdf.reindex(prov_order).fillna(0).reset_index(drop=True)
                for col in cdf_reindexed.columns:
                    xdf[col] = cdf_reindexed[col].values
            log.info(f"    Added 4 claim-level model features")
        except Exception as e:
            log.warning(f"    Claim-level model failed: {e}")
        ...
        # Clean up exhaustive search keys
        lgb_balance = lgb_p.pop("balance", "is_unbalance")
        lgb_p.pop("lgb_spw", None)
        bt = lgb_p.pop("boosting_type", "gbdt")
        dr = lgb_p.pop("drop_rate", None)
        lgb_p["boosting_type"] = bt
        if bt == "dart" and dr:
            lgb_p["drop_rate"] = dr
        if lgb_balance == "is_unbalance":
            lgb_p["is_unbalance"] = True
        elif lgb_balance == "scale_pos_weight":
            lgb_p["scale_pos_weight"] = pos_weight
        ...
        rf_p = dict(tuned_params.get("RandomForest", {
            "n_estimators": 500, "max_depth": 20, "max_features": "sqrt",
        }))
        rf_cw = rf_p.pop("class_weight", "balanced_subsample")
        rf_p.pop("use_smote", None)
        m_rf = RandomForestClassifier(**rf_p, class_weight=rf_cw, random_state=42, n_jobs=-1)
        ...
        # (Focal loss and DART were tested in scripts 12/13 - they hurt F1, removed)
    ...
    # Cohen's d effect size (supplements low-power Wilcoxon)
    effect_sizes = {}
    best_vals = np.array(all_results[best_key]["f1"]["values"])
    for name in stat_names:
        if name == best_key:
            continue
        other_vals = np.array(all_results[name]["f1"]["values"])
        diff = best_vals - other_vals
        cohens_d = diff.mean() / (diff.std() + 1e-9) if diff.std() > 0 else 0.0
        effect_sizes[name] = float(cohens_d)
    ...
    for name in sorted(corrected_p):
        ...
        d = effect_sizes.get(name, 0)
        d_label = "large" if abs(d) > 0.8 else "medium" if abs(d) > 0.5 else "small"
        log.info(f"    vs {name:25s}: p={corrected_p[name]:.4f} [{sig}] d={d:.3f} ({d_label})")
    ...
    log.info(f"  Saved: final_results.json")

    # Summary
    log.info(f"\n{'='*70}")
    log.info("FINAL SUMMARY (LEAKAGE-FREE + CLAIM MODEL + TARGET ENCODING)")
    log.info(f"{'='*70}")
    log.info(f"\n  {'Model':<35s} {'F1':>8s}")
    log.info(f"  {'-'*35} {'-'*8}")
